...
<head>
    ...
    <title>Tabel Mencari Kerja</title>
    ...
    <div class="content transition">
        <div class="container-fluid dashboard">
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Table Mencari Kerja</h5>
                        <?php if (isset($_GET['pesan'])) { ?>
                            <div class="alert alert-warning alert-dismissible fade show" role="alert">
                                <?php echo htmlspecialchars($_GET['pesan']); ?>
                                <button type="button" class="close" onclick="window.location.href='mencrkrj.php';" data-dismiss="alert" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                        <?php } ?>
                        <div class="mb-3">
                            <a href="crudmencrkrj/tambah.php" class="btn btn-primary btn-icon"><i class="fas fa-plus"></i> Tambah Data</a>
                            <a href="crudmencrkrj/print.php" target="_blank" class="btn btn-info btn-icon"><i class="fas fa-print"></i> Print</a>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-striped mb-0">
                                ...
                                <tbody>
                                    ...
                                    $querydata = mysqli_query($koneksi, "SELECT * FROM mencari_kerja ORDER BY id DESC limit $main_page, $limit");
                                    while ($log = mysqli_fetch_assoc($querydata)) { ?>
                                        <tr>
                                            <td><?php echo $log['nama']; ?></td>
                                            <td><?php echo $log['jenis_kelamin']; ?></td>
                                            <td><?php echo $log['alamat']; ?></td>
                                            <td><?php echo $log['alasan_cari_kerja']; ?></td>
                                            <td><a target="_blank" href="https://api.whatsapp.com/send?phone=<?= $log['kontak']; ?>"><?= $log['kontak']; ?></a></td>
                                            <?php if ($log['gambar'] != "") { ?>
                                                <td><a href="crudmencrkrj/lihat.php?id=<?php echo $log['id']; ?>"><img src="crudmencrkrj/gambar/<?php echo $log['gambar']; ?>" style="width:50px;"></a></td>
                                            <?php } ?>
                                            <?php if ($log['gambar'] == "") { ?>
                                                <td>Tidak ada gambar</td>
                                            <?php } ?>
                                            <td class="text-center"><a href="crudmencrkrj/edit.php?id=<?php echo $log['id']; ?>" class="btn btn-warning btn-icon"><i class="fas fa-edit"></i> Edit</td>
                                            <td class="text-center"><a href="crudmencrkrj/hapus.php?id=<?php echo $log['id']; ?>" class="btn btn-danger btn-icon" onclick="return confirm('Yakin ingin hapus data?')"><i class="fas fa-trash-alt"></i> Hapus</td>
                                        </tr>
                                    <?php
                                    }
                                    ?>
